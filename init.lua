...
local releaseInfo = HttpService:JSONDecode(game:HttpGetAsync("https://api.github.com/repos/" .. user .. "/nilware/releases"))[1]
...
if readFile and writeFile then
    ...
    if not ran or releaseInfo.tag_name ~= result then
        if hasFolderFunctions then
            ...
            createFolder("nilware")
            createFolder("nilware/user")
            createFolder("nilware/user/" .. user)
            createFolder("nilware/user/" .. user .. "/methods")
            createFolder("nilware/user/" .. user .. "/modules")
            createFolder("nilware/user/" .. user .. "/objects")
            createFolder("nilware/user/" .. user .. "/ui")
            createFolder("nilware/user/" .. user .. "/ui/controls")
            createFolder("nilware/user/" .. user .. "/ui/modules")
        end
        ...
        function environment.import(asset)
            ...
            elseif web then
                if readFile and writeFile then
                    local file = (hasFolderFunctions and "nilware/user/" .. user .. '/' .. asset .. ".lua") or ("nilware-" .. user .. '-' .. asset:gsub('/', '-') .. ".lua")
                    local content

                    if (isFile and not isFile(file)) or not importCache[asset] then
                        content = game:HttpGetAsync("https://raw.githubusercontent.com/" .. user .. "/nilware/" .. branch .. '/' .. asset .. ".lua")
                    ...
                    assets = { loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/" .. user .. "/nilware/" .. branch .. '/' .. asset .. ".lua"), asset .. '.lua')() }
                end
            else
                assets = { loadstring(readFile("nilware/" .. asset .. ".lua"), asset .. '.lua')() }
            end
